...
export function buildWriterPrompt(input: WriterPromptInput): string {
  ...
  parts.push(`\n### ⚠️ 章节衔接要求（最高优先级）`)
  parts.push(`- 本章正文必须从记忆编排上下文里的"章节连续性锚点"直接继续，不得跳过时间线`)
  parts.push(`- 如果锚点中包含"开篇承诺"，前300-500字必须先处理该承诺，再进入新地点或新时间状态`)
  parts.push(`- 必须按网文连载链路写：接钩 -> 短兑现/反转 -> 新阻碍升级 -> 本章目标 -> 章尾再钩`)
  parts.push(`- 开篇段落应自然承接上一章的最后一个场景/对话/情绪，而不是重新用环境描写开场`)
  parts.push(`- 如果上一章结尾是选择/系统提示/倒计时/问题，必须写出主角的选择、反馈、拖延或被迫中断`)
