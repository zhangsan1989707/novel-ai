...
export function buildPlannerPrompt(input: PlannerPromptInput): string {
  ...
  parts.push(`\n## ⚠️ 章节衔接要求（最高优先级）`)
  parts.push(`- 读取记忆编排上下文中的"章节连续性锚点"和"开篇承诺"，本章必须从这里直接延续`)
  parts.push(`- 必须按网文连载链路设计：接钩 -> 短兑现/反转 -> 新阻碍升级 -> 本章目标 -> 章尾再钩`)
  parts.push(`- 如果上一章结尾为悬念/悬崖（hook），本章的开场场景必须解决或深化该悬念`)
  parts.push(`- 如果上一章结尾是选择/系统提示/倒计时/问题，第一场景必须写清楚主角的选择、反馈、拖延或被迫中断`)
  ...
  parts.push(`\n### 开篇钩子设计（占章节前10%字数）`)
  parts.push(`开篇不是重开一章，而是先兑现上一章章尾承诺，再制造本章新压力。`)
